	  		...
	  		if ($_GET['upBook'] == "UPDATE BOOK") {
	  			...
	  			if (!empty($_GET['b'])) {
	  				...
	  				foreach ($op_values as $val) {
	  					if ($val == 'BName') {
	  						$NewBName = $_GET['BookName'];
	  						$sql1 = "UPDATE u_books SET BName = '$NewBName' WHERE UEmail = '".$_SESSION['UEmail']."' AND BName = '$BN' AND BAuthor = '$BA' AND BPrice = '$BP'";
	  						//echo "<br>$sql1";
	  						if (mysqli_query($conn,$sql1)) {
	  							echo "<br><h4>Book Name Updated Sucessfully!</h4><br>";
	  						}
	  						else{
	  							echo "<br><h4>Oops...Something went wrong!<br>Book Name not Updated!</h4><br>";
								echo "Error:".mysqli_error($conn);
	  						}
	  					}
	  					if ($val == 'BAuth') {
	  						$NewAuth = $_GET['BookAuthor'];
	  						$BName = $_GET['BookName'];
	  						$sql2 = "UPDATE u_books SET BAuthor = '$NewAuth' WHERE UEmail = '".$_SESSION['UEmail']."' AND BName = '$BN' AND BAuthor = '$BA' AND BPrice = '$BP'";
	  						//echo "<br>$sql2";
	  						if (mysqli_query($conn,$sql2)) {
	  							echo "<br><h4>Book Author Updated Sucessfully!</h4><br>";
	  						}
	  						else{
	  							echo "<br><h4>Oops...Something went wrong!<br>Book Author not Updated!</h4><br>";
	  							echo "Error:".mysqli_error($conn);
	  						}
	  					}
	  					if ($val == 'BPrice') {
	  						$NewBPrice = $_GET['BookPrice'];
	  						$BName = $_GET['BookName'];
	  						$sql3 = "UPDATE u_books SET BPrice = '$NewBPrice' WHERE UEmail = '".$_SESSION['UEmail']."' AND BName = '$BN' AND BAuthor = '$BA' AND BPrice = '$BP'";
	  						//echo "<br>$sql3";
	  						if (mysqli_query($conn,$sql3)) {
	  							echo "<br><h4>Book Price Updated Sucessfully!</h4><br>";
	  						}
	  						else{
	  							echo "<br><h4>Oops...Something went wrong!<br>Book Price not Updated!</h4><br>";
	  							echo "Error:".mysqli_error($conn);
	  						}
	  					}
	  				}
	  				...
	  				echo "<br><h4>No Checkbox Selected.</h4><br>";
	  				echo "<br><h4>No Book Updated!</h4><br>";
	  				echo "<br><h4>Redirecting you to Dashboard.....</h4><br>";	
