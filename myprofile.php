...
<body>
	...
	<?php
		...
		if(isset($_SESSION['UName'])){
			$selimg = "SELECT * FROM u_imgs WHERE UEmail='".$_SESSION['UEmail']."' ORDER BY U_id DESC LIMIT 1";
			$imgres = mysqli_query($conn,$selimg);
			$imgfile = $imgres -> fetch_array(MYSQLI_ASSOC);
			if(mysqli_num_rows($imgres) > 0){
	?>
		<img src="USERIMG/<?php echo $imgfile['UImg']; ?>" class="rounded-circle" height="200" width="200" alt="Your Profile Picture">
	<?php
			}
			$conn -> close();
		}
	...
	?>
		<img src="IMG/user.png" class="rounded-circle" height="200" width="200" alt="Your Profile Picture">
	<?php
		}
	?>
			<form method="POST" action="changeimg.php">
				<input type="submit" name="imgsub" id="imgsub" value="CHANGE PROFILE IMAGE">
			</form><br>
	<?php
		if(isset($_SESSION['UName'])){
			if ($_SESSION['UName']) {
			include ('dbconnect.php');
			$res = mysqli_query($conn,"SELECT * FROM users WHERE UEmail='".$_SESSION['UEmail']."' AND UPass='".$_SESSION['UPass']."'");
			$row = mysqli_fetch_array($res);	
	?>
	  <h4>NAME :&nbsp&nbsp&nbsp&nbsp <?php echo $row['UName']; ?></h4><br>
	<?php
			}
		}
		else{
	?>
	  <h4>NAME :&nbsp&nbsp&nbsp&nbsp Invalid User!</h4><br>
	<?php
		}
		if(isset($_SESSION['UName'])){
			if($_SESSION['UEmail']){
	?>
		<h4>EMAIL ID :&nbsp&nbsp&nbsp&nbsp <?php echo $row['UEmail']; ?></h4><br>
	<?php
			}
		}
		else{
	?>
		<h4>EMAIL ID :&nbsp&nbsp&nbsp&nbsp Invalid User!</h4><br>
	<?php
		}
		if(isset($_SESSION['UName'])){
			include ('dbconnect.php');
			...
			$row = mysqli_fetch_array($res);
			if($row['UGender'] != null){
				echo "<h4>GENDER :&nbsp&nbsp&nbsp&nbsp".$row['UGender']."</h4><br>";
				echo "<h4>Mobile Number :&nbsp&nbsp&nbsp&nbsp".$row['UMob']."</h4><br>";
			}
			$conn -> close();
		}
		else{
			echo "<h4>GENDER :&nbsp&nbsp&nbsp&nbsp Invalid User!</h4><br>";
			echo "<h4>Mobile Number :&nbsp&nbsp&nbsp&nbsp Invalid User!</h4><br>";
		}
	?>
		<form method="GET" action="updateprof.php">
			<input type="submit" name="upSub" id="upSub" value="UPDATE PROFILE">
		</form>
	</div>
	</div>
	</div>
